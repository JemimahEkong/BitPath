...
export class MessageService {
  ...
  async generateAiResponseStream(
    ...
    try {
      ...
      const xpAmount = 5;
      this.queueService.addXpJob({
        userId,
        xpAmount,
        reason: 'AI interaction',
      }).catch((err) => this.logger.error('Failed to add XP job:', err));
      this.queueService.addRewardJob({
        userId,
        conversationId,
        xpEarned: xpAmount,
        btcReward: 0,
        reason: 'AI interaction',
      }).catch((err) => this.logger.error('Failed to add Reward job:', err));
    } catch (error) {
