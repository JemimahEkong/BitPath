  ...
  implements OnModuleInit, OnModuleDestroy
{
  ...
  async onModuleInit() {
    try {
      ...
      this.logger.log(
        'Database connection established, skipping immediate test',
      );
      ...
      if (process.env.NODE_ENV === 'production') {
        this.logger.warn(
          'Database connection failed in production, continuing startup...',
        );
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (attempt < maxRetries && isConnectionError) {
          ...
          this.logger.log(
            `Waiting ${delay}ms before retry due to connection error...`,
          );
    ...
    setTimeout(() => {
      this.connectionCheckInterval = setInterval(async () => {
        try {
          ...
          if (!isHealthy) {
            this.logger.warn(
              'Database connection unhealthy, attempting to reconnect...',
            );
            try {
              await this.$disconnect();
